...
const billCache = new InProcessSnapshotCache<Cached<RiigikoguBillSummary[]>>(30 * 60_000, 5 * 60_000);
const memberCache = new InProcessSnapshotCache<Cached<RiigikoguFactionSummary[]>>(6 * 60 * 60_000, 15 * 60_000);
...
async function refreshBills(): Promise<Cached<RiigikoguBillSummary[]>> {
  const response = await fetchRiigikoguJson("/api/volumes/drafts", {
    membership: "15", proceedingStatus: "IN_PROCESS", lang: "ET",
...
async function refreshMembers(): Promise<Cached<RiigikoguFactionSummary[]>> {
  const response = await fetchRiigikoguJson("/api/plenary-members", { membership: "15", lang: "ET" });
  return { data: parseCurrentFactions(response.value), retrievedAt: response.retrievedAt };
}
...
export async function loadRiigikoguOverview(): Promise<RiigikoguOverviewResponse> {
  const results = await Promise.allSettled([
    ...
    billCache.get(refreshBills), memberCache.get(refreshMembers),
  ]);
  ...
  const retrieved = results.flatMap((result) => result.status === "fulfilled" ? [result.value.value.retrievedAt] : []).sort();
  return {
    membership: 15,
