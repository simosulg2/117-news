...
function createTitleFingerprint(title: string): TitleFingerprint {
  const normalized = normalizeStoryTitle(title);
...
function fingerprintsHaveConflictingNumbers(left: TitleFingerprint, right: TitleFingerprint): boolean {
  if (left.numbers.size === 0 || right.numbers.size === 0) return false;
  return ![...left.numbers].some((number) => right.numbers.has(number));
}

function fingerprintsShareMeaningfulAnchor(
...
function fingerprintSimilarity(left: TitleFingerprint, right: TitleFingerprint): number {
  return (
...
/**
 * Groups recent cross-source duplicates without transitive chaining. Articles
 * are considered only against a group's fixed (and newest) primary headline.
 */
export function groupNewsItems(items: NewsArticle[], now: number | Date): NewsItem[] {
  ...
  for (const candidate of ranked) {
    let matchingGroup: WorkingGroup | undefined;
    ...
    if (candidate.isRecent) {
      matchingGroup = groups.find((group) => {
        ...
        if (fingerprintsHaveConflictingNumbers(group.primary.fingerprint, candidate.fingerprint)) return false;
        if (!fingerprintsShareMeaningfulAnchor(group.primary.fingerprint, candidate.fingerprint)) return false;
        return fingerprintSimilarity(group.primary.fingerprint, candidate.fingerprint) >= STORY_GROUPING_THRESHOLD;
      });
    }
    ...
    groups.push({
      primary: candidate,
      related: [],
      sources: new Set([candidate.article.source]),
    });
  }
